...
create table if not exists public.purchases (
  ...
  payment_method text not null check (payment_method in ('who','stripe','crypto','paypal')),
...
ALTER TABLE public.videos ADD COLUMN IF NOT EXISTS is_free boolean NOT NULL DEFAULT false;

-- Add Whop API key column to site_config (migration for existing databases)
ALTER TABLE public.site_config ADD COLUMN IF NOT EXISTS who_api_key text;

-- Add PayPal column to site_config (only Client ID needed)
ALTER TABLE public.site_config ADD COLUMN IF NOT EXISTS paypal_client_id text;
